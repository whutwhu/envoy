...
// Enable case-sensitive VHDS query. When enabled (default), VHDS queries use the original
// case-sensitive host header value. When disabled, host headers are lowercased before VHDS query.
// Note: Domain matching is always case-insensitive regardless of this flag.
RUNTIME_GUARD(envoy_reloadable_features_vhds_case_sensitive_query);
